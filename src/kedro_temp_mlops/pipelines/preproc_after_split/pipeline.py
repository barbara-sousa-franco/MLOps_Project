...
def create_pipeline(**kwargs) -> Pipeline:
    return pipeline([
        node(
            func=impute_missing,
            inputs=["X_train_data", "X_test_data", "params:preproc_after_split"],
            outputs=["X_train_imputed", "X_test_imputed", "imputer"],
            name="impute_missing",
        ),
        node(
            func=cap_outliers,
            inputs=["X_train_imputed", "X_test_imputed", "params:preproc_after_split"],
            outputs=["X_train_processed", "X_test_processed", "capper"],
            name="cap_outliers",
        ),
        node(
            func=encode_categoricals,
            inputs=["X_train_processed", "X_test_processed", "y_train_data", "params:preproc_after_split"],
            outputs=["X_train_encoded", "X_test_encoded", "target_encoder"],
            name="encode_categoricals",
        ),
        node(
            func=scale_features,
            inputs=["X_train_encoded", "X_test_encoded", "params:preproc_after_split"],
